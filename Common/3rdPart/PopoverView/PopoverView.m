...
static float const kPopoverViewCellHeight = 45.0; ///< cell指定高度
...
@property (nonatomic, strong) PopoverViewCell *currentSelectCell;
...
- (void)hide {
    ...
    if (_currentButton) {
        UIImage *img = [_currentButton.currentImage BBSImageRotation:UIImageOrientationDown];
        [_currentButton setImage:img forState:UIControlStateNormal];
    }
...
- (void)showToView:(UIView *)pointView withActions:(NSArray<PopoverAction *> *)actions button:(UIButton *)button{
...
/*! @brief 指向指定的点来显示弹窗 */
- (void)showToPoint:(CGPoint)toPoint withActions:(NSArray<PopoverAction *> *)actions button:(UIButton *)button{
    _currentButton = button;
...
//    }
    ...
    PopoverViewCell *selectCell = [tableView cellForRowAtIndexPath:indexPath];
