...
- (void)layoutSubviews {
    ...
    if (self.bottomViewIsAlignment) {
        ...
        CGSize labelSize = [self sizeWithText:self.titles[0] font:segementTitleNormalFont maxSize:CGSizeMake(LBkScreen_width, self.height)];
 ...
 *  初始化静止标题栏（不可左右拖动）
 */
- (instancetype)initStaticTitlesWithFrame:(CGRect)frame titleFontSize:(CGFloat)titleSize{
...
//        self.bottomView.width = self.currentItem.width + (differenceW * (1 - self.currentSelectProgress));
        CGSize labelSize = [self sizeWithText:self.titles[currentIndex] font:segementTitleNormalFont maxSize:CGSizeMake(LBkScreen_width, self.height)];
        self.bottomView.width = labelSize.width;
        // 两个标题项中x的差
        CGFloat differenceX = self.nextItem.left - self.currentItem.left;
        // 设置动画修改x值
        self.bottomView.left = self.currentItem.left + (differenceX * (1 - self.currentSelectProgress)) + (self.currentItem.width - self.bottomView.width) / 2;
