...
- (void)configureUI
{
    ...
    [self.nickNameLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.avagtarImageView.mas_bottom).with.offset(30);
...
//    [self.nickNameLabel setFont:[UIFont systemFontOfSize:14]];
    ...
    [self.femaleButton setImage:[UIImage BBSImageNamed:@"/User/GenderDeselected.png"] forState:UIControlStateNormal];
...
//        make.right.equalTo(self.femaleButton.mas_left).with.offset(-25);
    ...
    [maleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(self.mas_centerX).with.offset(-80);
        make.centerY.mas_equalTo(self.femaleButton.mas_centerY);
        make.size.mas_equalTo(CGSizeMake(30, 15));
    }];
    ...
    [self.maleButton setImage:[UIImage BBSImageNamed:@"/User/GenderDeselected.png"] forState:UIControlStateNormal];
    ...
    [self.secretButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.mas_centerX).with.offset(85);
        make.centerY.mas_equalTo(self.femaleButton.mas_centerY);
        make.size.mas_equalTo(CGSizeMake(20, 20));
    }];
    [self.secretButton addTarget:self action:@selector(genderClickHandler:) forControlEvents:UIControlEventTouchUpInside];
    [self.secretButton setImage:[UIImage BBSImageNamed:@"/User/GenderDeselected.png"] forState:UIControlStateNormal];
    ...
    [self.commitButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(horizontalSeperateButtonView.mas_bottom).with.offset(40);
        make.left.mas_equalTo(@15);
        make.right.mas_equalTo(@(-15));
        make.height.mas_equalTo(@42);
    }];
    [self.commitButton setBackgroundColor:DZSUIColorFromHex(0x50A3D3)];
    ...
    if (self.editType == BBSUIEditUserInfoTypeRegister) {
        ...
        [self.lateSettingButton setTitleColor:DZSUIColorFromHex(0x50A3D3) forState:UIControlStateNormal];
...
- (void)genderClickHandler:(UIButton *)button
{
    ...
    [self.currentGenderButton setImage:[UIImage BBSImageNamed:@"/User/GenderDeselected.png"] forState:UIControlStateNormal];
    [button setImage:[UIImage BBSImageNamed:@"/User/GenderSelected.png"] forState:UIControlStateNormal];
    ...
    if ([fileManager fileExistsAtPath:BBSUIUserAvatarTmpPath]) {
        ...
        [BBSSDK uploadAvatarWithContentPath:BBSUIUserAvatarTmpPath scales:@[@48, @120, @200] result:^(NSArray *urls, NSError *error) {
            ...
            if (!error) {
                ...
                [BBSUIProcessHUD showFailInfo:@"上传头像失败"];
            }
...
- (void)editUserInfo:(NSDictionary *)avatarUrlDic
{
    //修改用户信息
    __weak typeof(self) theEidtView = self;
    [BBSSDK editUserInfoWithGender:self.genderIndex token:_currentUser.token avatarBigUrl:avatarUrlDic[@"avatarBig"] avatarMiddleUrl:avatarUrlDic[@"avatarMiddle"] avatarSmallUrl:avatarUrlDic[@"avatarSmall"] result:^(BBSUser *user, NSError *error) {
        
        [theEidtView.activeityIndicatorView removeFromSuperview];
        theEidtView.activeityIndicatorView = nil;
        
        if (!error) {
            user.groupName = [BBSUIContext shareInstance].currentUser.groupName;
            [BBSUIContext shareInstance].currentUser = user;
            
        }else{
            
            if (error.code == 900111) {
                [BBSUIProcessHUD showFailInfo:@"登录信息过期，请重新登录后设置"];
            }else{
                [BBSUIProcessHUD showFailInfo:@"更新用户信息失败"];
            }
