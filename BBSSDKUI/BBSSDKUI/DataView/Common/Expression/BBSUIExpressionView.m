...
- (void)layoutSubviews
{
    self.expressionSelectSV.frame = CGRectMake(0, CGRectGetMaxY(self.collectionView.frame), self.collectionView.width, 42);
}
...
- (void)configure
{
    ...
    self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, self.width, self.height - expressionSelectSVH) collectionViewLayout:layout];
    ...
    self.expressionSelectSV = [[UIScrollView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.collectionView.frame), self.collectionView.width, expressionSelectSVH)];
    self.expressionSelectSV.backgroundColor = DZSUIColorFromHex(0xF2F3F7);
    for (int i = 0; i < [self.expressionDictionary allKeys].count; i ++)
    {
        ...
        if (i == 0)
        {
            ...
            kindName = @"default";
            btnColor = DZSUIColorFromHex(0xFFFFFF);
