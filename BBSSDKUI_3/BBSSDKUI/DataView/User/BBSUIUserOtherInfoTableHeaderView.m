...
- (void)configUI {
    //添加背景模糊效果
    [self bbs_addVisualEffectView:UserTypeMe];
...
- (void)setHeaderWithUser:(BBSUser *)currentUser
{
    ...
    {
        [self.avatarImageView sd_setImageWithURL:[NSURL URLWithString:currentUser.avatar ] placeholderImage:[UIImage BBSImageNamed:@"/User/AvatarDefault3.png"] options:SDWebImageRefreshCached];
        ...
        self.verifyImgObserver = [getter getImageWithURL:[NSURL URLWithString:urlString] result:^(UIImage *image, NSError *error) {
            ...
            if (error) {
                //self.avatarImageView.image = [UIImage BBSImageNamed:@"/User/AvatarDefault3.png"];
                [self bbs_backgroundImageWithImage:[UIImage BBSImageNamed:@"/User/AvatarDefault3.png"]];
            }else{
                //self.avatarImageView.image = image;
                [self bbs_backgroundImageWithImage:image];
            }
        }];
    }
    ...
    {
        self.originLabel.attributedText = [NSString bbs_stringWithString:currentUser.sightml fontSize:12 defaultColorValue:@"4E4F57" lineSpace:0 wordSpace:0];
    }
    else
    {
        self.originLabel.attributedText = [NSString bbs_stringWithString:currentUser.sightml fontSize:12 defaultColorValue:@"FFFFFF" lineSpace:0 wordSpace:0];
    }
