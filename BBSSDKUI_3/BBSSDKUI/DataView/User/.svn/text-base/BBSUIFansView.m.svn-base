...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_type == BBSUIFansTypeFirendsMe) {// 我的关注
        [self setAttentionButtonWithCell:cell :YES];
        
    }else{
        if (fans.isFollow.integerValue == 1) {
            [self setAttentionButtonWithCell:cell :YES];
        }else{
            [self setAttentionButtonWithCell:cell :NO];
        }
    }
...
- (void)setAttentionButtonWithCell:(BBSUIFansTableViewCell *)cell :(BOOL)isAttention
{
    if (isAttention) // 已关注
    {
        [cell.attentionButton setTitle:@"已关注" forState:UIControlStateNormal];
        [cell.attentionButton setTitleColor:DZSUIColorFromHex(0xFFAA42) forState:UIControlStateNormal];
        [cell.attentionButton setBackgroundImage:nil forState:UIControlStateNormal];
        [cell.attentionButton setImage:[UIImage BBSImageNamed:@"/User/Confirm@2x.png"] forState:UIControlStateNormal];
        cell.attentionButton.layer.borderColor = DZSUIColorFromHex(0xFFAA42).CGColor;
        cell.attentionButton.layer.borderWidth = 0.5;
    }
    else
    {
        [cell.attentionButton setImage:nil forState:UIControlStateNormal];
        NSArray *colroArray = @[DZSUIColorFromHex(0xFF8D65), DZSUIColorFromHex(0xFFB85B)];
        [cell.attentionButton displayButtonImageFromColors:colroArray ByGradientType:leftToRight];
