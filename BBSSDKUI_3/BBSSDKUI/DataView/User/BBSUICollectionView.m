...
- (void)_getThreadList
{
    ...
    [BBSSDK getUserThreadListWithAuthorid:nil pageIndex:self.currentIndex pageSize:BBSUIPageSize result:^(NSArray<BBSThread *> *array, NSError *error) {
        ...
        if (weakSelf.currentIndex == 1) {
            ...
            [self.collectionTableView configureTipViewWithFrame:(CGRect){0, tipBackViewY, DZSUIScreen_width, self.frame.size.height - 370} tipMessage:@"暂无内容" noDataImage:nil  hasData:weakSelf.marrData.count != 0 hasError:YES reloadButtonBlock:^(id sender) {
                [weakSelf.collectionTableView.mj_header beginRefreshing];
                [weakSelf requestData];
            }];
            
            
//            [_tipBackView configureTipViewWithTipMessage:@"暂无内容" hasData:weakSelf.marrData.count != 0 hasError:YES reloadButtonBlock:^(id sender) {
//                [weakSelf.collectionTableView.mj_header beginRefreshing];
//                [weakSelf requestData];
//            }];
        }
...
- (void)_getFavorites
{
    ...
    [BBSSDK getUserThreadFavoritesWithPageIndex:self.currentIndex pageSize:BBSUIPageSize result:^(NSArray<BBSThread *> *array, NSError *error) {
        ...
        if (weakSelf.currentIndex == 1) {
            ...
            [weakSelf.collectionTableView configureTipViewWithFrame:CGRectMake(0, tipBackViewY, DZSUIScreen_width, self.frame.size.height - 370) tipMessage:@"暂无内容" noDataImage:nil  hasData:weakSelf.marrData.count != 0 hasError:YES reloadButtonBlock:^(id sender) {
