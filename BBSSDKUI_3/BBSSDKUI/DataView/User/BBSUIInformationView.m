...
- (void)requestData {
    ...
    [BBSSDK getNotificationsWithPageIndex:self.currentIndex pageSize:BBSUIPageSize result:^(NSArray<BBSInformation *> *array, NSError *error) {
        ...
        if (weakSelf.currentIndex == 1) {
            [weakSelf configureTipViewWithFrame:self.bounds tipMessage:@"暂无消息" noDataImage:[UIImage BBSImageNamed:@"/Common/noInformation@2x.png"] hasData:weakSelf.marrData.count != 0 hasError:YES reloadButtonBlock:^(id sender) {
