        ...
        [BBSSDK getPostListWithFid:fid tid:tid authorId:authorID pageIndex:page pageSize:pageSize result:^(NSArray *postList, NSError *error) {
            ...
            for (BBSPost *obj in postList)
            {
                if ([obj isKindOfClass:[BBSPost class]])
                {
                    ...
                    NSMutableDictionary *res = [[NSMutableDictionary alloc] initWithDictionary:postDic];
                    if ([BBSSDK isUsePlug])
                    {
                        res[@"isPlug"] = @1;
                    }
...
#pragma mark -----ssssss
            //=====喜欢加+1
            NSLog(@"-----llxxx-论坛喜欢--%ld", (long)_threadModel.recommend_add);
            _threadModel.recommend_add =  _threadModel.recommend_add  + 1;
            [[BBSUICoreDataManage shareManager] addHistoryWithThread:_threadModel];
            NSLog(@"-----llxxx-论坛喜欢2--%ld", (long)_threadModel.recommend_add);
...
- (void)_saveLikedThread
{
    ...
    NSLog(@"-----tid-----%@", @(self.threadModel.tid));
    NSLog(@"------dic----%@", likedThreadDictionary);
...
- (void)postCommentWithHTML:(NSString *)html pid:(NSInteger)pid
{
    ...
    float lat = 0;
    float lng = 0;
    NSDictionary *locationInfo = self.commentTextView.addressLBS;
    BBSLocation *location = nil;
    if (locationInfo) {
        address = locationInfo[@"address"];
        poiTitle = locationInfo[@"name"];
...
- (void)postCommentSuccess:(BBSPost *)post prePid:(NSInteger)pid comment:(NSString *)comment
{
    ...
    NSLog(@"----ssss---0000--%ld", (long)_threadModel.commentnum);
    
    _threadModel.replies = _threadModel.replies + 1;
    // 添加历史记录
    [[BBSUICoreDataManage shareManager] addHistoryWithThread:_threadModel];
    
    NSLog(@"----ssss---11111---%ld", (long)_threadModel.commentnum);
