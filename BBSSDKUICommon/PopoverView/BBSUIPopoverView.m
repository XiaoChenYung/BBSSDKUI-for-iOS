...
static float const kBBSUIPopoverViewMargin = 8.f; ///< 边距
static float const kBBSUIPopoverViewCellHeight = 45.0; ///< cell指定高度
static float const kBBSUIPopoverViewArrowHeight = 13.f; ///< 箭头高度

// convert degrees to radians
float BBSUIDegreesToRadians(float angle) {
    return angle*M_PI/180;
}

@interface BBSUIPopoverView () <UITableViewDelegate, UITableViewDataSource>
...
#pragma mark - Data
@property (nonatomic, copy) NSArray<BBSUIPopoverAction *> *actions;
...
@property (nonatomic, strong) BBSUIPopoverViewCell *currentSelectCell;
@property (nonatomic, weak) UIImageView *arrowImageView;

@property (nonatomic, copy) void (^hiddenResult)();

@end

@implementation BBSUIPopoverView
...
- (void)layoutSubviews {
    ...
    _tableView.frame = CGRectMake(0, _isUpward ? kBBSUIPopoverViewArrowHeight : 0, CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds) - kBBSUIPopoverViewArrowHeight);
}
...
- (void)setStyle:(BBSUIPopoverViewStyle)style {
    _style = style;
    _tableView.separatorColor = [BBSUIPopoverViewCell bottomLineColorForStyle:_style];
...
- (void)initialize {
    ...
    _actions = @[];
    _isUpward = YES;
    _style = BBSUIPopoverViewStyleDefault;
...
#pragma mark ------ 如果箭头指向的点过于偏左或者过于偏右则需要重新调整箭头 x 轴的坐标======
    // 如果箭头指向的点过于偏左或者过于偏右则需要重新调整箭头 x 轴的坐标
    CGFloat minHorizontalEdge = kBBSUIPopoverViewMargin + cornerRadius + arrowWidth/2 + 2;
    if (toPoint.x < minHorizontalEdge) {
        toPoint.x = minHorizontalEdge;
    }
    //(CGPoint) toPoint = (x = 187.5, y = 69)
    ...
    CGFloat currentH = _tableView.contentSize.height + kBBSUIPopoverViewArrowHeight;
    
    // 限制最高高度, 免得选项太多时超出屏幕
    CGFloat maxHeight = _isUpward ? (_windowHeight - toPoint.y - kBBSUIPopoverViewMargin) : (toPoint.y - CGRectGetHeight([UIApplication sharedApplication].statusBarFrame));
    ...
    // x: 窗口靠左
    if (toPoint.x <= currentW/2 + kBBSUIPopoverViewMargin) {
        currentX = kBBSUIPopoverViewMargin;
    }
    // x: 窗口靠右
    if (_windowWidth - toPoint.x <= currentW/2 + kBBSUIPopoverViewMargin) {
        currentX = _windowWidth - kBBSUIPopoverViewMargin - currentW;
    }
    // y: 箭头向下
    if (!_isUpward) {
    ...
    CGFloat maskTop = _isUpward ? kBBSUIPopoverViewArrowHeight : 0; // 顶部Y值
    CGFloat maskBottom = _isUpward ? currentH : currentH - kBBSUIPopoverViewArrowHeight; // 底部Y值
    ...
    [maskPath addArcWithCenter:CGPointMake(cornerRadius, cornerRadius + maskTop)
                        radius:cornerRadius
                    startAngle:BBSUIDegreesToRadians(180)
                      endAngle:BBSUIDegreesToRadians(270)
                     clockwise:YES];
    // 箭头向上时的箭头位置
    if (_isUpward) {
        [maskPath addLineToPoint:CGPointMake(arrowPoint.x - arrowWidth/2, kBBSUIPopoverViewArrowHeight)];
        [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x - arrowCornerRadius, arrowCornerRadius)
                         controlPoint:CGPointMake(arrowPoint.x - arrowWidth/2 + arrowBottomCornerRadius, kBBSUIPopoverViewArrowHeight)];
        [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x + arrowCornerRadius, arrowCornerRadius)
                         controlPoint:arrowPoint];
        [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x + arrowWidth/2, kBBSUIPopoverViewArrowHeight)
                         controlPoint:CGPointMake(arrowPoint.x + arrowWidth/2 - arrowBottomCornerRadius, kBBSUIPopoverViewArrowHeight)];
    }
    // 右上圆角
    ...
    [maskPath addArcWithCenter:CGPointMake(currentW - cornerRadius, maskTop + cornerRadius)
                        radius:cornerRadius
                    startAngle:BBSUIDegreesToRadians(270)
                      endAngle:BBSUIDegreesToRadians(0)
                     clockwise:YES];
    // 右下圆角
    ...
    [maskPath addArcWithCenter:CGPointMake(currentW - cornerRadius, maskBottom - cornerRadius)
                        radius:cornerRadius
                    startAngle:BBSUIDegreesToRadians(0)
                      endAngle:BBSUIDegreesToRadians(90)
                     clockwise:YES];
    // 箭头向下时的箭头位置
    if (!_isUpward) {
        [maskPath addLineToPoint:CGPointMake(arrowPoint.x + arrowWidth/2, currentH - kBBSUIPopoverViewArrowHeight)];
        [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x + arrowCornerRadius, currentH - arrowCornerRadius)
                         controlPoint:CGPointMake(arrowPoint.x + arrowWidth/2 - arrowBottomCornerRadius, currentH - kBBSUIPopoverViewArrowHeight)];
        [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x - arrowCornerRadius, currentH - arrowCornerRadius)
                         controlPoint:arrowPoint];
        [maskPath addQuadCurveToPoint:CGPointMake(arrowPoint.x - arrowWidth/2, currentH - kBBSUIPopoverViewArrowHeight)
                         controlPoint:CGPointMake(arrowPoint.x - arrowWidth/2 + arrowBottomCornerRadius, currentH - kBBSUIPopoverViewArrowHeight)];
    }
    // 左下圆角
    ...
    [maskPath addArcWithCenter:CGPointMake(cornerRadius, maskBottom - cornerRadius)
                        radius:cornerRadius
                    startAngle:BBSUIDegreesToRadians(90)
                      endAngle:BBSUIDegreesToRadians(180)
                     clockwise:YES];
    [maskPath closePath];
    // 截取圆角和箭头
...
- (CGFloat)calculateMaxWidth {
    CGFloat maxWidth = 0.f, titleLeftEdge = 0.f, imageWidth = 0.f, imageMaxHeight = kBBSUIPopoverViewCellHeight - BBSUIPopoverViewCellVerticalMargin*2;
    CGSize imageSize = CGSizeZero;
    UIFont *titleFont = [BBSUIPopoverViewCell titleFont];
    for (BBSUIPopoverAction *action in _actions) {
        imageWidth = 0.f;
        titleLeftEdge = 0.f;
        
        if (action.image) { // 存在图片则根据图片size和图片最大高度来重新计算图片宽度
            titleLeftEdge = BBSUIPopoverViewCellTitleLeftEdge; // 有图片时标题才有左边的边距
...
#pragma GCC diagnostic pop
        ...
        CGFloat contentWidth = BBSUIPopoverViewCellHorizontalMargin*2 + imageWidth + titleLeftEdge + titleWidth;
    ...
    if (maxWidth > CGRectGetWidth(_keyWindow.bounds) - kBBSUIPopoverViewMargin*2) {
        maxWidth = CGRectGetWidth(_keyWindow.bounds) - kBBSUIPopoverViewMargin*2;
    }
...
- (void)hide:(UIButton *)sender {
    ...
    if ([_arrowImageView isKindOfClass:[UIButton class]]) {
        UIButton *arrowButton = (UIButton *)_arrowImageView;
        [arrowButton setImage:[arrowButton.imageView.image BBSImageRotation:UIImageOrientationDown] forState:UIControlStateNormal];
    }else if ([_arrowImageView isKindOfClass:[UIImageView class]]){
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return kBBSUIPopoverViewCellHeight;
}

static NSString *kPopoverCellIdentifier = @"kPopoverCellIdentifier";
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    BBSUIPopoverViewCell *cell = [tableView dequeueReusableCellWithIdentifier:kPopoverCellIdentifier];
    if (!cell) {
        cell = [[BBSUIPopoverViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kPopoverCellIdentifier];
    }
...
//    if (self.orderIndex == indexPath.row) {
...
//    }
    ...
    if (self.orderIndex == indexPath.row) {
        //[cell changeSelectStatus:YES];
        [cell changeSelectStatus:YES isFashion:self.isFashion];
    }else{
        //[cell changeSelectStatus:NO];
        [cell changeSelectStatus:NO isFashion:self.isFashion];
    }
...
//    }
    ...
    [UIView animateWithDuration:0.25f animations:^{
        ...
        BBSUIPopoverAction *action = _actions[indexPath.row];
    ...
    BBSUIPopoverViewCell *selectCell = [tableView cellForRowAtIndexPath:indexPath];
    [selectCell changeSelectStatus:YES];
    if (self.currentSelectCell) {
        [self.currentSelectCell changeSelectStatus:NO];
    }
}
