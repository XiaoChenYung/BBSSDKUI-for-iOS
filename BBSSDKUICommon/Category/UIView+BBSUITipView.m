...
#import "Masonry.h"
#import "UIColor+BBSUICommon.h"
#import "UIButton+BBSUICommon.h"
#import "UIImage+BBSFunction.h"

@interface UIView ()

@property (nonatomic, strong) BBSUITipView *tipView;

@end

@implementation UIView (BBSUITipView)

- (void)setTipView:(BBSUITipView *)tipView
{
...
- (BBSUITipView *)tipView
{
    return objc_getAssociatedObject(self, _cmd);
}

- (void)bbs_configureTipViewWithTipMessage:(NSString *)tipString hasData:(BOOL)hasData
{
    [self bbs_configureTipViewWithTipMessage:tipString
                                 hasData:hasData
                             noDataImage:[UIImage BBSImageNamed:@"/Common/wnr3@2x.png"]];
}

- (void)bbs_configureTipViewWithTipMessage:(NSString *)tipString
...
- (void)bbs_configureTipViewWithFrame:(CGRect)frame
                ...
                reloadButtonBlock:(void (^)(id sender))block
{
    ...
    [self bbs_configureTipViewWithFrame:frame
...
- (void)bbs_configureTipViewWithError:(NSError *)error
                          hasData:(BOOL)hasData
                reloadButtonBlock:(void (^)(id))block
{
    if (hasData)
    ...
    else
    {
        if (!self.tipView)
        {
            self.tipView = [[BBSUITipView alloc] initWithFrame:self.bounds];
        }
        self.tipView.hidden = NO;
        [self.bbs_tipViewContainer addSubview:self.tipView];
                ...
                reloadButtonBlock:(void (^)(id))block
{
    ...
    else
    {
        if (!self.tipView)
        {
            self.tipView = [[BBSUITipView alloc] initWithFrame:frame];
        }
        self.tipView.hidden = NO;
        [self.bbs_tipViewContainer addSubview:self.tipView];
