...
@implementation UIColor (BBSUICommon)

+ (instancetype)bbs_fromHexValue:(NSUInteger)hex
{
    NSUInteger a = ((hex >> 24) & 0x000000FF);
    float fa = ((0 == a) ? 1.0f : (a * 1.0f) / 255.0f);
    
    return [UIColor bbs_fromHexValue:hex alpha:fa];
}

+ (instancetype)bbs_fromHexValue:(NSUInteger)hex alpha:(CGFloat)alpha
{
...
+ (instancetype)bbs_fromShortHexValue:(NSUInteger)hex alpha:(CGFloat)alpha
{
...
+ (instancetype)bbs_colorFormString:(NSString *)string
{
    ...
    if ([color hasPrefix:@"#"]) // #FFF
    {
        ...
        if (color.length == 3)
        {
            ...
            return [UIColor bbs_fromShortHexValue:hexRGB alpha:alpha];
        }
        else if (color.length == 6)
        {
            NSUInteger hexRGB = strtol(color.UTF8String, nil, 16);
            return [UIColor bbs_fromHexValue:hexRGB alpha:alpha];
        }
    }
    else if ([color hasPrefix:@"0x"] || [color hasPrefix:@"0X"]) // #FFF
    {
        ...
        {
            NSUInteger hexRGB = strtol(color.UTF8String, nil, 16);
            return [UIColor bbs_fromHexValue:hexRGB];
        }
        else if (color.length == 6)
        {
            NSUInteger hexRGB = strtol(color.UTF8String, nil, 16);
            return [UIColor bbs_fromHexValue:hexRGB alpha:1.0f];
        }
    }
    else
    {
