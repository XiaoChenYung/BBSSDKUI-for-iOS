...
void BBSUIHSVColorFromRGBColor(const BBSUIHRRGBColor* rgb,BBSUIHRHSVColor* hsv){
    BBSUIHRRGBColor rgb255 = {rgb->r * 255.0f,rgb->g * 255.0f,rgb->b * 255.0f};
    BBSUIHRHSVColor hsv255 = {0.0f,0.0f,0.0f};
...
void RGBColorFromBBSUIHSVColor(const BBSUIHRHSVColor* hsv,BBSUIHRRGBColor* rgb){
   /*
    UIColorには
...
void RGBColorFromUIColor(const UIColor* uiColor,BBSUIHRRGBColor* rgb){
...
int HexColorFromRGBColor(const BBSUIHRRGBColor* rgb){
    return (int)(rgb->r*255.0f) << 16 | (int)(rgb->g*255.0f) << 8 | (int)(rgb->b*255.0f) << 0;
}

int HexColorFromUIColor(const UIColor* color){
    BBSUIHRRGBColor rgb_color;
    RGBColorFromUIColor(color, &rgb_color);
    return HexColorFromRGBColor(&rgb_color);
}

bool BBSUIHRHSVColorEqualToColor(const BBSUIHRHSVColor* hsv1,const BBSUIHRHSVColor* hsv2){
    return (hsv1->h == hsv2->h) && (hsv1->s == hsv2->s) && (hsv1->v == hsv2->v);
}

void BBSUIHSVColorAt(BBSUIHRHSVColor* hsv,float x,float y,float saturationUpperLimit,float brightness){
    hsv->h = x;
