...
#import "BBSUIRTEBaseTextView.h"
#import "BBSUIRTELayoutManager.h"
#import "BBSUIRTETextStorage.h"

#define RGB(r,g,b) [UIColor colorWithRed:r/255.0f green:g/255.0f blue:b/255.0f alpha:1.0f]

static void *BBSUIRTETextViewContext = &BBSUIRTETextViewContext;
static const float kCursorVelocity = 1.0f/8.0f;

@interface BBSUIRTEBaseTextView ()

@property (nonatomic, strong) BBSUIRTELayoutManager *lineNumberLayoutManager;
@property (nonatomic, strong) BBSUIRTETextStorage *syntaxTextStorage;

@end

@implementation BBSUIRTEBaseTextView
{
    NSRange startRange;
}
...
{
    BBSUIRTETextStorage *textStorage = [BBSUIRTETextStorage new];
    BBSUIRTELayoutManager *layoutManager = [BBSUIRTELayoutManager new];
...
{
    // Setup observers
    [self addObserver:self forKeyPath:NSStringFromSelector(@selector(font)) options:NSKeyValueObservingOptionNew context:BBSUIRTETextViewContext];
    [self addObserver:self forKeyPath:NSStringFromSelector(@selector(selectedTextRange)) options:NSKeyValueObservingOptionNew context:BBSUIRTETextViewContext];
    [self addObserver:self forKeyPath:NSStringFromSelector(@selector(selectedRange)) options:NSKeyValueObservingOptionNew context:BBSUIRTETextViewContext];
...
{
    if ([keyPath isEqualToString:NSStringFromSelector(@selector(font))] && context == BBSUIRTETextViewContext)
    {
        ...
        self.syntaxTextStorage.defaultFont = self.font;
              ...
              [keyPath isEqualToString:NSStringFromSelector(@selector(selectedRange))]) && context == BBSUIRTETextViewContext)
    {
...
- (NSArray *)tokens
{
    BBSUIRTETextStorage *syntaxTextStorage = (BBSUIRTETextStorage *)self.textStorage;
