...
#import "BBSUIHRBrightnessCursor.h"
#import "BBSUIHRColorCursor.h"

@interface BBSUIHRColorPickerView()
...
@implementation BBSUIHRColorPickerView
...
+ (BBSUIHRColorPickerStyle)fitScreenStyle
{
    CGSize defaultSize = [[UIScreen mainScreen] applicationFrame].size;
    defaultSize.height -= 44.f;
    
    BBSUIHRColorPickerStyle style = [BBSUIHRColorPickerView defaultStyle];
...
{
    BBSUIHRColorPickerStyle style = [BBSUIHRColorPickerView fitScreenStyle];
    style.brightnessLowerLimit = 0.0f;
    style.saturationUpperLimit = 1.0f;
    return style;
}
...
{
    return [self initWithStyle:[BBSUIHRColorPickerView defaultStyle] defaultColor:defaultColor];
}

- (id)initWithStyle:(BBSUIHRColorPickerStyle)style defaultColor:(const BBSUIHRRGBColor)defaultColor{
    CGSize size = [BBSUIHRColorPickerView sizeWithStyle:style];
    ...
    if (self) {
        ...
        // RGBのデフォルトカラーをHSVに変換
        BBSUIHSVColorFromRGBColor(&_defaultRgbColor, &_currentHsvColor);
        ...
        _brightnessCursor = [[BBSUIHRBrightnessCursor alloc] initWithPoint:CGPointMake(_brightnessPickerFrame.origin.x, _brightnessPickerFrame.origin.y + _brightnessPickerFrame.size.height/2.0f)];
        
        // タイルの中心にくるようにずらす
        _colorCursor = [[BBSUIHRColorCursor alloc] initWithPoint:CGPointMake(_colorMapFrame.origin.x - ([BBSUIHRColorCursor cursorSize].width - _tileSize)/2.0f - [BBSUIHRColorCursor shadowSize]/2.0,
                                                                        _colorMapFrame.origin.y - ([BBSUIHRColorCursor cursorSize].height - _tileSize)/2.0f - [BBSUIHRColorCursor shadowSize]/2.0)];
...
- (BBSUIHRRGBColor)RGBColor{
    BBSUIHRRGBColor rgbColor;
    RGBColorFromBBSUIHSVColor(&_currentHsvColor, &rgbColor);
    return rgbColor;
}
...
- (void)createCacheImage{
    ...
    BBSUIHRSetRoundedRectanglePath(brightness_picker_shadow_context,
...
- (void)update{
    ...
    if (_isDragging || _isDragStart || _isDragEnd || _isTapped) {
        ...
        if (CGRectContainsPoint(_colorMapFrame,touchPosition)) {
            ...
            BBSUIHRHSVColor newHsv = _currentHsvColor;
            ...
            BBSUIHSVColorAt(&newHsv, pixelX, pixelY, _saturationUpperLimit, _currentHsvColor.v);
            
            if (!BBSUIHRHSVColorEqualToColor(&newHsv,&_currentHsvColor)) {
...
- (void)updateColorCursor{
    ...
    BBSUIHRRGBColor currentRgbColor = [self RGBColor];
...
- (void)drawRect:(CGRect)rect
{
    ...
    BBSUIHRSetRoundedRectanglePath(context, _brightnessPickerFrame, 5.0f);
    ...
    BBSUIHRRGBColor darkColor;
    BBSUIHRRGBColor lightColor;
    ...
    BBSUIHRHSVColor pixelHsv;
    BBSUIHRRGBColor pixelRgb;
    for (int j = 0; j < pixelCountY; ++j) {
        ...
        for (int i = 0; i < pixelCountX; ++i) {
            ...
            BBSUIHSVColorAt(&pixelHsv, pixelX, pixelY, _saturationUpperLimit, _currentHsvColor.v);
            RGBColorFromBBSUIHSVColor(&pixelHsv, &pixelRgb);
    ...
    CGContextSaveGState(context);
    BBSUIHRDrawSquareColorBatch(context, CGPointMake(CGRectGetMidX(_currentColorFrame), CGRectGetMidY(_currentColorFrame)), &currentRgbColor, _currentColorFrame.size.width/2.0f);
    CGContextRestoreGState(context);
...
- (void)setDelegate:(NSObject<BBSUIHRColorPickerViewDelegate>*)picker_delegate{
